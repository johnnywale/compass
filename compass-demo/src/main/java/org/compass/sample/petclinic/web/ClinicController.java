 ...
 * @author Ken Krebs
 */
public class ClinicController extends MultiActionController implements
		InitializingBean {
	...
	public void afterPropertiesSet() throws Exception {
		if (clinic == null)
			throw new ApplicationContextException(
					"Must set clinic bean property on " + getClass());
	}
	 ...
	 * @return a ModelAndView to render the response
	 */
	public ModelAndView tutorialHandler(HttpServletRequest request,
			HttpServletResponse response) throws ServletException {
	 ...
	 * @return a ModelAndView to render the response
	 */
	public ModelAndView springTutorialHandler(HttpServletRequest request,
			HttpServletResponse response) throws ServletException {
		return new ModelAndView("spring-tutorial");
	}
	...
	public ModelAndView vetsHandler(HttpServletRequest request,
			HttpServletResponse response) throws ServletException {
		return new ModelAndView("vets", "vets", clinic.getVets());
	}
	...
	public ModelAndView ownerHandler(HttpServletRequest request,
			HttpServletResponse response) throws ServletException {
		Owner owner = clinic.loadOwner(RequestUtils.getIntParameter(request,
				"ownerId", 0));
		if (owner == null) {
		...
		return new ModelAndView("owner", "model", model);
	}
}
