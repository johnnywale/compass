...
public class LibraryTests extends TestCase {
	...
	protected void setUp() throws Exception {
		...
		CompassConfiguration config = new CompassConfiguration()
				.configure("/org/compass/sample/library/compass.cfg.xml")
				.addClass(Author.class).addClass(Article.class)
				.addClass(Book.class);
	...
	private void assertJackLondon(Author author) {
		...
		assertEquals(sdf.format(jackLondon.getBirthdate()),
				sdf.format(author.getBirthdate()));
		assertEquals(jackLondon.getName().getFirstName(), author.getName()
				.getFirstName());
		assertEquals(jackLondon.getName().getLastName(), author.getName()
				.getLastName());
		assertEquals(2, author.getBooks().size());
		assertEquals("White Fang", ((Book) author.getBooks().get(0)).getTitle());
		assertEquals("The Call of the Wild",
				((Book) author.getBooks().get(1)).getTitle());
	}
	...
	public void setUpData() throws Exception {
		...
		whiteFang
				.setSummary("The remarkable story of a fiercely independent creature of the wild");
		whiteFang
				.setKeywords(new String[] { "jack london", "call of the wild" });
		...
		callOfTheWild
				.setSummary("The Call of the Wild is a tale about unbreakable spirit");
		callOfTheWild.setKeywords(new String[] { "jack london", "buck",
				"white fang" });
		...
		jamesClavell.setKeywords(new String[] { "far east", "shogun", "japan",
				"hong kong" });
		...
		shogun.setKeywords(new String[] { "james clavell", "Blackthorne",
				"Toranaga", "japan" });
		...
		taipan.setKeywords(new String[] { "james clavell", "Dirk Struan",
				"joss", "hong kong" });
	...
	public static void print(CompassHits hits, int hitNumber) {
		...
		System.out.println("ALIAS [" + resource.getAlias() + "] ID ["
				+ ((Identifiable) value).getId() + "] SCORE ["
