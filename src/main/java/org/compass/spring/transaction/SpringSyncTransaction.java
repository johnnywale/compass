...
public class SpringSyncTransaction extends AbstractTransaction {
	...
	private static final Log log = LogFactory
			.getLog(SpringSyncTransaction.class);

	private TransactionStatus status;

	/**
	 * This flag means if this transaction controls the COMPASS transaction
	 * (i.e. it is the top level compass transaction)
	 */
	...
	public void begin(PlatformTransactionManager transactionManager,
			InternalCompassSession session, boolean commitBeforeCompletion) {
		...
		if (transactionManager != null) {
			...
			int timeout = session.getSettings().getSettingAsInt(
					CompassEnvironment.Transaction.TRANSACTION_TIMEOUT, -1);
			if (timeout != -1) {
		...
		if (transactionManager != null) {
			...
			sync = new SpringTransactionSynchronization(session,
					status.isNewTransaction(), commitBeforeCompletion,
					transactionFactory);
		} else {
			if (log.isDebugEnabled()) {
				log.debug("Joining Spring transaction, and starting a new compass transaction on thread ["
			...
			sync = new SpringTransactionSynchronization(session, false,
					commitBeforeCompletion, transactionFactory);
		}
	...
	public void join(InternalCompassSession session) throws CompassException {
		...
		if (log.isDebugEnabled()) {
			log.debug("Joining an existing compass transcation on thread ["
					+ Thread.currentThread().getName() + "]");
		}
	}
	...
	public static class SpringTransactionSynchronization implements
		...
		public SpringTransactionSynchronization(InternalCompassSession session,
				boolean compassControledTransaction,
		...
		public void afterCompletion(int status) {
			try {
				...
				((SpringSyncTransactionFactory) transactionFactory)
						.unbindSessionFromTransaction(this, session);
				session.evictAll();
