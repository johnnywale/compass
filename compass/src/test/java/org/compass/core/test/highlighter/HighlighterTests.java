...
public class HighlighterTests extends AbstractTestCase {
	...
	protected void addSettings(CompassSettings settings) {
		...
		settings.setGroupSettings(
				LuceneEnvironment.Highlighter.PREFIX,
				"smallFragmenter",
				new String[] { LuceneEnvironment.Highlighter.Fragmenter.SIMPLE_SIZE },
				new String[] { "20" });
		settings.setGroupSettings(LuceneEnvironment.Highlighter.PREFIX,
				"commaSeparator",
				new String[] { LuceneEnvironment.Highlighter.SEPARATOR },
				new String[] { "," });
		settings.setGroupSettings(
				LuceneEnvironment.QueryParser.PREFIX,
				"noConstantScorePrefix",
				new String[] {
						LuceneEnvironment.QueryParser.TYPE,
						LuceneEnvironment.QueryParser.DEFAULT_PARSER_ALLOW_CONSTANT_SCORE_PREFIX_QUERY },
				new String[] { DefaultLuceneQueryParser.class.getName(),
						"false" });
	}
	...
	public void testSimpleHighlighting() {
		...
		assertEquals(fragment, detachedHits.highlightedText(0)
				.getHighlightedText());

		fragment = hits.highlighter(1).setHighlighter("smallFragmenter")
				.setMaxNumFragments(3).fragmentsWithSeparator("text");
		assertEquals(
				"This piece of text refers to <b>Kennedy</b>... to <b>Kennedy</b>",
				fragment);
		assertEquals(fragment, hits.highlightedText(1).getHighlightedText());
		detachedHits = hits.detach();
		assertEquals(fragment, detachedHits.highlightedText(1)
				.getHighlightedText());

		String fragments[] = hits.highlighter(1)
				.setHighlighter("smallFragmenter").setMaxNumFragments(3)
				.fragments("text");
		assertEquals(2, fragments.length);
		assertEquals("This piece of text refers to <b>Kennedy</b>",
				fragments[0]);
		...
		hits = session.queryBuilder().queryString("Kenn*")
				.setQueryParser("noConstantScorePrefix").toQuery().hits();
		fragment = hits.highlighter(0).fragment("text");
		assertEquals("John <b>Kennedy</b> has been shot", fragment);
	...
	public void testNoTermVectorException() {
		...
		String fragment = hits.highlighter(1)
				.setTextTokenizer(CompassHighlighter.TextTokenizer.ANALYZER)
				.setHighlighter("smallFragmenter").setMaxNumFragments(3)
				.fragmentsWithSeparator("text");
		assertEquals(
				"This piece of text refers to <b>Kennedy</b>... to <b>Kennedy</b>",
				fragment);
		...
		try {
			hits.highlighter(0)
					.setTextTokenizer(
							CompassHighlighter.TextTokenizer.TERM_VECTOR)
					.fragment("text");
			fail();
	...
	public void testWithTermVectorException() {
		...
		fragment = hits.highlighter(1)
				.setTextTokenizer(CompassHighlighter.TextTokenizer.TERM_VECTOR)
				.setHighlighter("smallFragmenter").setMaxNumFragments(3)
				.fragmentsWithSeparator("text");
		assertEquals(
				"This piece of text refers to <b>Kennedy</b>... to <b>Kennedy</b>",
	...
	public void testMultiResourceHighlighter() {
		...
		setUpMultiPropertyData(session, new String[] { "Lucene", "Luke",
				"Lukas" });
		...
		String fragment = hits.highlighter(0).setHighlighter("commaSeparator")
	...
	public void testSimpleHighlightingWithAlias() {
		...
		CompassHits hits = session.queryBuilder().queryString("bla")
				.setQueryParser("noConstantScorePrefix").toQuery()
				.setAliases("parent").hits();
		String fragment = hits.highlighter(0).fragment("text");
