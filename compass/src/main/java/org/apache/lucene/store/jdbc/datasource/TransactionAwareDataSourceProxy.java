 ...
 * <p/>
 * Wraps the created Jdbc <code>Connection</code> with a {@link ConnectionProxy}
 ...
 * <p/>
 * A set of simple utilities are provided in the {@link DataSourceUtils} for
 * simpler management of the <code>DataSource</code>, and special care is taken
 * if the <code>DataSource</code> uses the {@link ConnectionProxy} (such is the
 * case with this data soruce). For example, the
 * {@link DataSourceUtils#commitConnectionIfPossible(java.sql.Connection)} and
 * {@link DataSourceUtils#rollbackConnectionIfPossible(java.sql.Connection)}
 ...
 * {@link TransactionAwareDataSourceProxy} is there to help non managed
...
public class TransactionAwareDataSourceProxy implements DataSource {
	...
	public Connection getConnection(String username, String password)
		...
		if (con == null) {
			...
			return getTransactionAwareConnectionProxy(con,
					getTargetDataSource(), true);
		}
	 ...
	 * <p/>
	 * Consider using
	 * {@link DataSourceUtils#getConnection(javax.sql.DataSource)} and
	 * {@link DataSourceUtils#releaseConnection(java.sql.Connection)} for
	 * simpler usage.
	 */
	...
	protected Connection getTransactionAwareConnectionProxy(Connection target,
			DataSource dataSource, boolean controllsConnection) {
	...
	public <T> T unwrap(Class<T> iface) throws SQLException {
		try {
			Method method = dataSource.getClass().getMethod("unwarp",
					Class.class);
	...
	public boolean isWrapperFor(Class<?> iface) throws SQLException {
		try {
			Method method = dataSource.getClass().getMethod("isWrapperFor",
					Class.class);
			return (Boolean) method.invoke(dataSource, iface);
		} catch (Exception e) {
			throw new SQLException("Failed to invoke isWrapperFor "
					+ e.getMessage());
		}
	}
	...
	private static class TransactionAwareInvocationHandler implements
		...
		public TransactionAwareInvocationHandler(Connection target,
				DataSource dataSource, boolean controlConnection) {
			this.target = target;
			this.dataSource = dataSource;
				...
				if (controlConnection) {
					...
					if (holders == null || !holders.containsKey(dataSource)) {
						throw new IllegalStateException(
								"No value for data source [" + dataSource
										+ "] bound to thread ["
										+ Thread.currentThread().getName()
										+ "]");
					}
					Connection transConnection = (Connection) holders
							.remove(dataSource);
					if (holders.isEmpty()) {
